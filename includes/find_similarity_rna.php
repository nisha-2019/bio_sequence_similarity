...
?>
<body>
<form id="form1" name="form1" method="post" action=".?action=do_find_sim_rna">
  <center><p><strong>FIND RNA SIMILARITY</strong></p></center>
  <table width="516" align="center">
    <tr>
      <td width="104">RNA 1</td>
      <td colspan="2"><span id="spryselect1">
        <label for="r1"></label>
        <select name="r1" id="r1">
          <option value="-1">SELECT</option>
          <?php
		  $sql="select * from protein where type like 'RNA' and oid=$_GET[o1]";
    ...
    <tr>
      ...
      <td colspan="2"><span id="spryselect2">
        ...
        <select name="r2" id="r2">
          <option value="-1">SELECT</option>
          <?php
		  $sql="select * from protein where type like 'RNA' and  oid=$_GET[o2]";
		  $res=mysql_query($sql,$con);
		  while($row=mysql_fetch_array($res))
		  {
